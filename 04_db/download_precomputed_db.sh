...
SCORES_PATH="${DEST}/${SCORES_FILE}"
log "all files verified."
log ""
log "NEXT STEP: point config.yaml at these files and run the pipeline."
log "  ctx_db_fname             : ${RANKINGS_PATH}"
log "  dem_db_fname             : ${SCORES_PATH}"
log "  path_to_motif_annotations: ${DEST}/${MOTIF2TF_FILE}"
log ""
log "All three live in this one directory, which is what"
log "03_pipeline/config.template.yaml expects."
log ""
log "The peak-overlap audit does NOT need to be re-run for this reference: it"
log "was run on the real peaks (81.20% of 393,832 representable) and the"
log "decision is recorded in docs/DATABASE_DECISION.md -- the precomputed"
log "database is adequate here. Re-run the audit only for a DIFFERENT peak set:"
log "  python 04_db/peaks_to_bed.py --h5ad <atac.h5ad> --out peaks.bed"
log "  python 04_db/peak_overlap_audit.py --peaks peaks.bed \\"
log "      --db '${RANKINGS_PATH}' --out-prefix 04_db/audit_screen"
log ""
log "Context for that number: SCREEN regions are cCREs of 150-350 bp (median"
log "272) covering chr1-22,X,Y only, against 500 bp peaks here. pycistarget"
log "passes a pair if EITHER overlap fraction clears 0.4, so the narrower"
log "database region is the easier route, not a handicap. Peaks matching no"
log "cCRE are dropped from motif enrichment with NO warning."
