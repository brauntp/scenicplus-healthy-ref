    ...
    if fail is None:
        ...
        if not fail["fails"]:
            ...
            if any(f.get("enomem") for f in fail["fails"]):
                ...
                print()
                print("  For the AUCell rules the fix is --cpus-per-task 1, NOT")
                print("  more memory. Measured: 128G and 192G failed with a")
                print("  byte-identical traceback, on a 400 GB+ node, while")
                print("  enriching the 3.2 GB GENE matrix -- so size was never")
                print("  the constraint. Under strict overcommit the kernel")
                print("  charges each fork's worst case rather than trusting")
                print("  copy-on-write, and a ~81 GB parent cannot fork 16 ways")
                print("  at any --mem.")
                print()
                print("  aucell4r takes a SERIAL branch at num_workers == 1:")
                print("  no RawArray, no Process, no fork. Run AUCell alone:")
                print("    sbatch --cpus-per-task=1 slurm/scenicplus.sbatch \\")
                print("        --target AUCell_direct.h5mu --target AUCell_extended.h5mu")
            if len(fail["fails"]) > 1:
                print()
